Shader "Custom Shader/Reflection"
{
    Properties
    {
        _MainTexture ("Main Texture", 2D) = "grey" {}
        [NoScaleOffset] [Normal] _NormalMap ("Normal Map", 2D) = "bump" {}
        [NoScaleOffset] _ReflectionCubeMap ("Reflection Cube Map", Cube) = "white" {}
    ...
    SubShader
    {
        ...
        fixed4 calculateFinalColor(VertexOutput INPUT, fixed4 ambientColor) : SV_TARGET
        {
            ...
            fixed4 reflectionColor = 1;

            // light data
            half3 normal = normalize(UnpackNormal(UNITY_SAMPLE_TEX2D(_NormalMap, INPUT.uv)));
            ...
            reflectionColor = UNITY_SAMPLE_TEXCUBE(_ReflectionCubeMap, reflectionVector);
            // reflectionColor = UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, reflectionVector);

            return saturate(textureColor * reflectionColor * saturate(ambientColor + diffuseColor) + specularColor);
        }
        ENDHLSL

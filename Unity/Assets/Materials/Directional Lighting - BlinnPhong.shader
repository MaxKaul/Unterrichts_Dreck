Shader "Custom Shader/Directional Lighting - BlinnPhong"
{
    Properties
    {
    ...
    SubShader
    {
		Pass
		{
			...
			fixed4 PS(VertexOutput INPUT) : SV_TARGET
			{
				...
				// specular color
				half3 halfVector = normalize(light + viewDirection);
				float specular = pow(max(dot(normal, halfVector), 0), _SpecularPower) * _Glossy;
